...
class GetIron extends Command
{
     ...
     * @var string
     */
    protected $signature = 'api:get-iron';
    ...
    public function handle()
    {
        ...
        $response = $getjson->request('GET', 'XLON/IRON.json?api_key=UcX5xRwzEpUfqn7FQWbj', [
            'verify'=> false
        ]);
        ...
        $writer = Writer::createFromFileObject(new \SplFileObject(storage_path('commodities/iron.csv'),'w'));
        $writer->insertOne(['Value','Date']);
        $writer->insertAll($set_arr);

        $seed = new CsvSeeder($setb = 'iron');
