...
class GetCorn extends Command
{
     ...
     * @var string
     */
    protected $signature = 'api:get-corn';
    ...
    public function handle()
    {
        ...
        $response = $getjson->request('GET', 'LSE/CORN.json?api_key=UcX5xRwzEpUfqn7FQWbj'.$this->getDateRangeString(), [
            'verify'=> false
        ]);
        ...
        $writer = Writer::createFromFileObject(new \SplFileObject(storage_path('commodities\corn.csv'),'w'));
        $writer->insertOne(['Value','Date']);
        $writer->insertAll($set_arr);

        $seed = new CsvSeeder($setb = 'corn');
