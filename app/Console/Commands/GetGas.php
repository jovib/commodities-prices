...
class GetGas extends Command
{
     ...
     * @var string
     */
    protected $signature = 'api:get-gas';
    ...
    public function handle()
    {
        ...
        $response = $getjson->request('GET', 'YAHOO/GASS.json?api_key=UcX5xRwzEpUfqn7FQWbj'.$this->getDateRangeString(), [
            'verify'=> false
        ]);
        ...
        $writer = Writer::createFromFileObject(new \SplFileObject(storage_path('commodities/gas.csv'),'w'));
        $writer->insertOne(['Value','Date']);
        $writer->insertAll($set_arr);

        $seed = new CsvSeeder($setb = 'gas');
