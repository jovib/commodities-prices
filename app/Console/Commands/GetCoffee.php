...
class GetCoffee extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'api:get-coffee';
    ...
    public function handle()
    {
        ...
        $response = $getjson->request('GET', 'COM/COFFEE_CLMB.json?api_key=UcX5xRwzEpUfqn7FQWbj'.$this->getDateRangeString(), [
            'verify'=> false
        ]);
        ...
        $writer = Writer::createFromFileObject(new \SplFileObject(storage_path('commodities/coffee.csv'),'w'));
        $writer->insertOne(['Value','Date']);
        $writer->insertAll($set_arr);

        $seed = new CsvSeeder($setb = 'coffee');
