...
class GetCooper extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'api:get-copper';
     ...
     * @var string
     */
    protected $description = 'get cooper prices from api';
    ...
    protected function getDateRangeString()
    {
        ...
        $start_date = $current->subYears(1)->toDateString();
    ...
    public function handle()
    {
        ...
        $response = $getjson->request('GET', 'COM/WLD_COPPER.json?api_key=UcX5xRwzEpUfqn7FQWbj'.$this->getDateRangeString(), [
            'verify'=> false
        ]);
        ...
        $writer = Writer::createFromFileObject(new \SplFileObject(storage_path('commodities/copper.csv'),'w'));
        $writer->insertOne(['Value','Date']);
        $writer->insertAll($set_arr);

        $seed = new CsvSeeder($setb = 'copper');
